...
初期はAを中心とし、Bを月1件・年間12件含める。Cは主役にしない。
...
- 妊娠期／出生後の入口、任意の予定日入力、出生後更新
- 子どもの生年月日・性別入力。端末内保存とし、性別は未設定・初節句候補の変更・非表示が可能
- 家族固有行事の自動生成
- ホーム「つぎのお祝い」と簡易な時系列表示
- 行事詳細、意味、一般的な目安
- 家庭の予定日調整、「未定／やる予定／今回はやらない」
- 準備チェックリスト
- 家族固有行事・季節文化の種類別ローカル通知
- 月1件・年間12件の季節文化行事
- 季節文化の「やってみる／今回は見送る」
- 入力編集、通知変更、端末内データの全削除
- 行事に応じた「写真の準備を見る」等の中立的な準備カテゴリCTAと、アプリ内準備ガイド
...
`D-005`により、Appleカレンダー連携、簡易共有、体験記録、ウィジェットはValidation ReleaseのWon'tとする。入口・将来予告・権限要求も実装せず、Phase 2 / 2.5後の正式v1計画で再評価する。
...
- 予定日、生年月日、性別、行事情報は原則端末内に保存・処理
- 初期はアカウント登録なし
- 独自サーバーは原則なし
- Analyticsには個人を直接特定しない行動イベントだけを送る
- 予定日、生年月日、性別の正確値と、逆算・再識別可能な細粒度派生値をAnalyticsへ送信しない。粗粒度コホートの採否・定義はD-007〜009で決める
...
`D-001`、`P0-PROD-01`、`P0-VAL-01`は完了済み。Marketingの`P0-MKT-01/02/03/05`もProduct Leadのレビューを経て完了し、Audience / occasion、訴求仮説、Brand / positioning変更境界、Phase 2獲得仮説・ASO準備項目が揃った。

`D-002`は2026年8月16日に確定。MLP対象は、お七夜、お宮参り、お食い初め／百日祝い、ハーフバースデー、初節句、1歳祝いの6テーマと、生後1〜11か月の軽量な月齢マイルストーンとする。

`D-003`も2026年8月16日に確定。季節文化は月1件・年間12件とし、各月の最終対象を`P0-PROD-02`で品質・安全・地域差・制作負荷から評価する。
...
`P0-PROD-03`もValidation Leadの独立レビューを経て完了。妊娠期／出生後の入口から、行事生成、次の行事、意味、準備候補、通知候補、季節文化までのWireflow、画面候補、状態、例外・回復、検証可能性を`mlp-experience-map.md`へ整理した。

`D-004`は2026年8月16日に確定。妊娠期／出生後の入口、任意の予定日入力、生年月日・性別の端末内保存、行事生成、ホーム「つぎのお祝い」、時系列、家庭の予定日調整、最小判断状態、準備チェック、季節文化の選択、種類別ローカル通知、編集・全削除をMustとする。性別は初節句候補の初期表示に利用するが、未設定・候補変更・非表示を可能にし、正確値等をAnalyticsへ送信しない。準備カテゴリ別CTAは`D-017`へ留保する。

`D-005`も2026年8月16日に確定。Appleカレンダー連携、簡易共有、体験記録、ウィジェットはValidation ReleaseのWon'tとし、Phase 2 / 2.5後の正式v1計画で再評価する。

`D-017`も2026年8月16日に確定。未提携状態では「写真の準備を見る」等の中立CTAからアプリ内準備ガイドへ遷移し、予約・購入・外部送客・未提供機能を示唆しない。クリックは準備情報へのカテゴリ関心であり、購入意向・収益性の証拠にはせず、非クリック者、後続行動、定性証拠と組み合わせる。

`D-021`も2026年8月16日に確定。Phase 1は目標8週間／責任者224時間、ハード上限10週間または280時間の早い方、週28時間、追加開発費0円とする。外注、有料SDK、有料バックエンドを使わず、週次・4週目に完了予測を更新し、超過見込みでは新規実装を止めてスコープ削減またはHuman Ownerの再判断を行う。工数都合でMustや品質・計測・Legal Gateを無承認で削らない。Apple関連の必須費用は`D-012`で別途判断する。

`P0-PROD-04`もProduct / Validation / Legalの独立レビューを経て完了。D-002 / D-003の対象、D-004 / D-017のMust、D-005のWon't、D-013品質、D-021上限を[`mlp-frozen-scope.md`](workstreams/product/mlp-frozen-scope.md)へ統合し、Validation ReleaseのMay機能は0件とした。実装詳細を除き、変更には影響D-IDとFrozen ScopeのHuman Owner改定が必要である。次は`P0-PROD-05`およびData / Legal / Validationの判断・設計へ進む。
...
5. Go / Pivot / Stop条件を確定。開発上限は`D-021`で確定済み
