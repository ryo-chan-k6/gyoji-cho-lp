...
Appleカレンダー連携、簡易共有、体験記録、ウィジェットは`D-005`で判断する。
...
`D-004`は2026年8月16日に確定。妊娠期／出生後の入口、任意の予定日入力、生年月日・性別の端末内保存、行事生成、ホーム「つぎのお祝い」、時系列、家庭の予定日調整、最小判断状態、準備チェック、季節文化の選択、種類別ローカル通知、編集・全削除をMustとする。性別は初節句候補の初期表示に利用するが、未設定・候補変更・非表示を可能にし、Analyticsへ送信しない。準備カテゴリ別CTAは`D-017`へ留保する。次は`D-005`を反映後、`P0-PROD-04`でMust / May / Won'tを固定する。
