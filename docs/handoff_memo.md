...
`D-021`も2026年8月16日に確定。Phase 1は目標8週間／責任者224時間、ハード上限10週間または280時間の早い方、週28時間、追加開発費0円とする。外注、有料SDK、有料バックエンドを使わず、週次・4週目に完了予測を更新し、超過見込みでは新規実装を止めてスコープ削減またはHuman Ownerの再判断を行う。工数都合でMustや品質・計測・Legal Gateを無承認で削らない。Apple関連の必須費用は`D-012`で別途判断する。次は`P0-PROD-04`でD-004 / D-017のMust、D-005のWon't、D-021の上限をFrozen Scopeへ固定する。
