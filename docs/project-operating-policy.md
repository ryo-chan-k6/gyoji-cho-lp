...
- 当週のAI・広告・クラウド・外注費と上限残
- 責任者の確認待ち時間
- 次週のReady上限3件

Phase 0の具体的な期限・責任者稼働・費用上限は事業判断を含むため、次の判断IDで別Chatにて決める。

| ID | 判断事項 | 決める値 | 決まるまでの制約 |
|---|---|---|---|
| `D-018` | Phase 0の時間枠 | 目標終了日、見直し日 | 期限付きの完了予測を確定しない |
| `D-019` | 責任者の稼働枠 | 週あたり確認時間、判断SLA | 週次コミットメントを確定しない |
| `D-020` | Phase 0の費用上限 | AI、調査、ツール、外注の上限 | 新規の有料利用を開始しない |

広告費は `D-010`、Phase 2獲得費は `D-014`、投資上限と最終権限は `D-016` で扱う。
...
1. 本方針、`AGENTS.md`、テンプレート、自動チェックをリポジトリへ反映する
2. 責任者が `D-018〜020` を別Chatで確定する
