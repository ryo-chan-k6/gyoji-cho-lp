...
- Task ID：`P0-PROD-03`
- 版：v1.1（D-004反映、Validation Lead Review済み）
- 更新日：2026年8月16日
- 入力：[`Product validation brief`](product-validation-brief.md)、[`Event scope matrix`](event-scope-matrix.md)
- 関連判断：`D-002〜004`は確定済み。付加機能は`D-005`、算出・保存等の技術方式は`D-006`で決める

## 1. 目的と読み方

この文書は、iOS MLPの体験を画面、状態、例外、回復導線まで一続きに可視化する。v1.0では機能候補を比較し、v1.1で`D-004`のMust / Won'tを反映した。`D-005`の付加機能と`D-006`以降の方式・計測は引き続き未決定である。
...
| 継続利用 | つぎのお祝いと準備を知る | ホーム→詳細→判断→準備チェック→通知 | 予告と実際の慣習差、対象行事を行わない | CTAはD-017、観測条件はD-007〜009 |
...
| S09 | 準備支援 | やるつもりのお祝いの次の行動をチェックできる | チェック未着手／途中／完了、カテゴリ関心 | チェックはMust、関心CTAはD-017 |
...
| 準備支援 | チェック操作をMust。カテゴリ関心はD-017 | 「準備まで」の成立とPreparation Intent観測 |
...
|---|---|---|
| Product | D-004で確定した画面・Core path・状態・例外と、D-005判断単位 | `P0-PROD-04`、`P1-PROD-01〜02`、`D-005/006`、`D-017` |
| Data | 候補状態とJourneyの観測点。通知は許可→配信適格→配信成功→開封→復帰先有効→詳細・準備を分け、未許可・拒否・種類別Off・通知窓超過を配信不成功へ混在させない。予定日・生年月日・性別の正確値と、逆算・再識別可能な細粒度派生値をイベント名・属性として送信しない。粗粒度コホートの採否・定義はD-007〜009へ留保する | `P0-DATA-01〜03`、`D-007〜009` |
| Legal | 予定日は妊娠期の任意入力・一般的見通し、生年月日は個別行事算出、性別は未設定可・初節句候補の初期表示だけに目的限定する。3属性の端末内処理・Analytics禁止、編集、全削除、予約済み通知取消、通知設定、一般的目安をデータフローと説明へ引き継ぐ | `P0-LEGAL-01〜03`、`D-008` |
...
- D-004の決定を反映し、D-005 / D-006以降を先取りせず、残る判断単位を比較可能にしている
- D-005確定後、P0-PROD-04のMust / May / Won'tへ機械的に転記できる
